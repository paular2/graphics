 ...
 */
    ...
    */
    computeAABB(){
        ...
        {
            Xmin = Math.min(Xmin, this.vBuffer[3*i]);
            Xmax = Math.max(Xmax, this.vBuffer[3*i]);
            
            Ymin = Math.min(Ymin, this.vBuffer[3*i+1]);
            Ymax = Math.max(Ymax, this.vBuffer[3*i+1]);
            
            Zmin = Math.min(Zmin, this.vBuffer[3*i+2]);
            Zmax = Math.max(Zmax, this.vBuffer[3*i+2]);
        }
        
        this.minXYZ = [Xmin, Ymin, Zmin];
        this.maxXYZ = [Xmax, Ymax, Zmax];
        
        console.log("MIN:", this.minXYZ[0], this.minXYZ[1], this.minXYZ[2]);
        console.log("MAX:", this.maxXYZ[0], this.maxXYZ[1], this.maxXYZ[2]);
    }
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for(var i = 0; i < arr.length; i++)
        {
            if(arr[i].charAt(0) == '#')
            {
                console.log("Comments:", arr[i]);
            }
            if(arr[i].charAt(0) == 'v')
            {
                var arr2 = arr[i].split(' ');
                //console.log("Split String:", arr2[1]);
                //console.log("Split String:", arr2[2]);
                //console.log("Split String:", arr2[3]);
                
                this.vBuffer.push(parseFloat(arr2[1]));
                this.vBuffer.push(parseFloat(arr2[2]));
                this.vBuffer.push(parseFloat(arr2[3]));
                this.numVertices++;
                
            }
            if(arr[i].charAt(0) == 'f')
            {
                arr2 = arr[i].split(/\b\s+(?!$)/);
                this.fBuffer.push(parseInt(arr2[1]-1));
                this.fBuffer.push(parseInt(arr2[2]-1));
                this.fBuffer.push(parseInt(arr2[3]-1));
                this.numFaces++;
            }
        }
